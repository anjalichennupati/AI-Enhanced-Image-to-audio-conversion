...
parser.add_argument('--test', default='E:/ASEB/3rd Year/Signal and Image Processing/Dataset/test',
                    help='test mode, you need give the test pics dirs in this param')
...
def save_current_codes(des_path):
    # eg：/n/liyz/videosteganography/main.py
    main_file_path = os.path.realpath(__file__)
    cur_work_dir, mainfile = os.path.split(
        main_file_path)  # eg：/n/liyz/videosteganography/
...
def main():
    ...
    if opt.test == '':
        ...
        opt.Hnet = "E:/ASEB/3rd Year/Signal and Image Processing/training/LAPTOP-FJM61C1T_2023-12-16-11_58_23/checkPoints/netH_epoch_99,sumloss=0.001365,Hloss=0.000897.pth"
        opt.Rnet = "E:/ASEB/3rd Year/Signal and Image Processing/training/LAPTOP-FJM61C1T_2023-12-16-11_58_23/checkPoints/netR_epoch_99,sumloss=0.001365,Rloss=0.000623.pth"
    ...
    if opt.test == '':
        ...
        for epoch in range(opt.niter):
            # train ##########################################
            train(train_loader, epoch, Hnet=Hnet,
                  Rnet=Rnet, criterion=criterion)

            ####################### validation  #####################################
            val_hloss, val_rloss, val_sumloss = validation(
                val_loader, epoch, Hnet=Hnet, Rnet=Rnet, criterion=criterion)
...
def train(train_loader, epoch, Hnet, Rnet, criterion):
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        # get true batch size of this step
        ...
        # put concat_image into H-net and get container image
        container_img = Hnet(concat_imgv)
        # loss between cover and container
        ...
        # loss between secret image and revealed secret image
...
def test(test_loader, epoch, Hnet, Rnet, criterion):
    ...
    for i, data in enumerate(test_loader, 0):
        ...
        container_img = Hnet(concat_imgv)
        # H-net reconstructed error
        errH = criterion(container_img, cover_imgv)
        Hlosses.update(errH.item(), this_batch_size)

        # containerImg as input of R-net and get "rev_secret_img"
        rev_secret_img = Rnet(container_img)
        # secret_imgv as label of R-net
        with torch.no_grad():
            secret_imgv = Variable(secret_img)
        # R-net reconstructed error
        errR = criterion(rev_secret_img, secret_imgv)
        Rlosses.update(errR.item(), this_batch_size)
        save_result_pic(this_batch_size, cover_img, container_img.data, secret_img, rev_secret_img.data, epoch, i,
                        opt.testPics)
...
def save_result_pic(this_batch_size, originalLabelv, ContainerImg, secretLabelv, RevSecImg, epoch, i, save_path):
    if not opt.debug:
        originalFrames = originalLabelv.resize_(
            this_batch_size, 3, opt.imageSize, opt.imageSize)
        containerFrames = ContainerImg.resize_(
            this_batch_size, 3, opt.imageSize, opt.imageSize)
        ...
        resultImgName = '%s/ResultPics_epoch%03d_batch%04d.png' % (
            save_path, epoch, i)
        vutils.save_image(resultImg, resultImgName,
                          nrow=this_batch_size, padding=1, normalize=True)
